...
// fsTestCtx is a test context for filesystem storage tests.
var fsTestCtx = context.Background()
...
func TestNewFilesystem_CreatesBaseDir(t *testing.T) {
	...
	assert.False(t, fs.PasteExists(fsTestCtx, "nonexistent"))
}
...
func TestFilesystem_CreatePaste_Success(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, "f468483c313401e8", paste)
	require.NoError(t, err)

	// Verify paste exists
	assert.True(t, fs.PasteExists(fsTestCtx, "f468483c313401e8"))
}
...
func TestFilesystem_CreatePaste_DuplicateID_ReturnsError(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, "testpasteid1234", paste)
	require.NoError(t, err)
...
func TestFilesystem_ReadPaste_Success(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, pasteID, original)
	require.NoError(t, err)

	// Read it back
	read, err := fs.ReadPaste(fsTestCtx, pasteID)
	require.NoError(t, err)
...
func TestFilesystem_ReadPaste_NotFound(t *testing.T) {
	...
	_, err = fs.ReadPaste(fsTestCtx, "nonexistent12345")
...
func TestFilesystem_ReadPaste_Expired(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, pasteID, paste)
	require.NoError(t, err)

	// Should return expired error
	_, err = fs.ReadPaste(fsTestCtx, pasteID)
...
func TestFilesystem_DeletePaste_Success(t *testing.T) {
	...
	// Delete it
	err = fs.DeletePaste(fsTestCtx, pasteID)
	require.NoError(t, err)

	// Verify it's gone
	assert.False(t, fs.PasteExists(fsTestCtx, pasteID))
}
...
func TestFilesystem_DeletePaste_NotFound(t *testing.T) {
	...
	err = fs.DeletePaste(fsTestCtx, "nonexistent12345")
	assert.ErrorIs(t, err, model.ErrPasteNotFound)
}
...
func TestFilesystem_CreateComment_Success(t *testing.T) {
	...
	err = fs.CreateComment(fsTestCtx, pasteID, pasteID, "comment12345678", comment)
	require.NoError(t, err)

	// Verify comment exists
	assert.True(t, fs.CommentExists(fsTestCtx, pasteID, pasteID, "comment12345678"))
}
...
func TestFilesystem_CreateComment_PasteNotFound(t *testing.T) {
	...
	err = fs.CreateComment(fsTestCtx, "nonexistent12345", "nonexistent12345", "comment12345678", comment)
	assert.ErrorIs(t, err, model.ErrPasteNotFound)
}
...
func TestFilesystem_CreateComment_DuplicateID(t *testing.T) {
	...
	err = fs.CreateComment(fsTestCtx, pasteID, pasteID, commentID, comment)
	require.NoError(t, err)
...
func TestFilesystem_ReadComments_Success(t *testing.T) {
	...
	err = fs.CreateComment(fsTestCtx, pasteID, pasteID, "comment10000001", comment1)
	require.NoError(t, err)
	err = fs.CreateComment(fsTestCtx, pasteID, pasteID, "comment20000002", comment2)
	require.NoError(t, err)

	// Read all comments
	comments, err := fs.ReadComments(fsTestCtx, pasteID)
	require.NoError(t, err)
...
func TestFilesystem_CommentExists(t *testing.T) {
	...
	// Should exist
	assert.True(t, fs.CommentExists(fsTestCtx, pasteID, pasteID, commentID))

	// Should not exist
	assert.False(t, fs.CommentExists(fsTestCtx, pasteID, pasteID, "nonexistent1234"))
}
...
func TestFilesystem_SetValue_Success(t *testing.T) {
	...
	err = fs.SetValue(fsTestCtx, "test", "key1", "value1")
	require.NoError(t, err)

	// Verify we can read it back
	value, err := fs.GetValue(fsTestCtx, "test", "key1")
	require.NoError(t, err)
...
func TestFilesystem_GetValue_NotFound(t *testing.T) {
	...
	value, err := fs.GetValue(fsTestCtx, "nonexistent", "key")
...
func TestFilesystem_SetValue_Overwrite(t *testing.T) {
	...
	err = fs.SetValue(fsTestCtx, "test", "key", "original")
	require.NoError(t, err)

	err = fs.SetValue(fsTestCtx, "test", "key", "updated")
	require.NoError(t, err)

	value, err := fs.GetValue(fsTestCtx, "test", "key")
	require.NoError(t, err)
...
func TestFilesystem_GetExpiredPastes(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, "expired12345678", expiredPaste)
	require.NoError(t, err)
	...
	err = fs.CreatePaste(fsTestCtx, "valid123456789a", validPaste)
	require.NoError(t, err)
	...
	err = fs.CreatePaste(fsTestCtx, "never123456789a", neverPaste)
	require.NoError(t, err)

	// Get expired pastes
	expired, err := fs.GetExpiredPastes(fsTestCtx, 10)
	require.NoError(t, err)
...
func TestFilesystem_Purge(t *testing.T) {
	...
	err = fs.CreatePaste(fsTestCtx, "purge123456789a", expiredPaste)
	require.NoError(t, err)

	// Verify the paste exists before purge
	assert.True(t, fs.PasteExists(fsTestCtx, "purge123456789a"))

	// Purge - should delete at least 1 paste (our expired one)
	count, err := fs.Purge(fsTestCtx, 10)
	require.NoError(t, err)
	assert.GreaterOrEqual(t, count, 1)

	// Verify it's gone - this is the key assertion
	assert.False(t, fs.PasteExists(fsTestCtx, "purge123456789a"))
}
...
func TestFilesystem_DeletePaste_WithComments(t *testing.T) {
	...
	// Both should be gone
	assert.False(t, fs.PasteExists(fsTestCtx, pasteID))
	assert.False(t, fs.CommentExists(fsTestCtx, pasteID, pasteID, "comment12345678"))
}
